...
class EventLoisirController extends AbstractController
{
     ...
     */
    public function addevent(Request $request, AdminRepository $adminRepository, AdminEventController $adminEventController, EventLoisirRepository $eventLoisirRepository, AdminEventRepository $adminEventRepository){
        $session = $request->getSession();
        if($session->get('id')==null){
            return $this->redirectToRoute('SignIn');
        }else{
            ...
            if(($form->isSubmitted()) and ($form->isValid())){
                ...
                if($session->get('prenom') != null){
                    $freelancer=$emfr->find($session->get('id'));
                    $em->merge($freelancer);
                    $em->flush($freelancer);
