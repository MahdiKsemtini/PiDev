...
class AdminEmploiController extends AbstractController
{
    ...
    public function index(PaginatorInterface $paginator,Request $request,OffreEmploiRepository $offreEmploiRepository,OffreStageRepository $offreStageRepository): Response
    {
        ...
        $em2=$this->getDoctrine()->getRepository(OffreStage::class);
        $list=$em->findAll();
        $countStage=$em2->countNbOffre();
    ...
    public function OffreEmploiToAdmin(AdminRepository $adminRepository, $idOffreEmploi,$SocieteEmail,OffreEmploiRepository $offreEmploiRepository,AdminEmploiRepository $adminEmploiRepository)
    {
        ...
        foreach ($list as $l) {
            ...
            $this->notify_creation->notifyOffreEmploi($SocieteEmail, $l->getLogin());
        }
    }
     ...
     */
    public function DeactivateOffreEmploi(DemandeEmploiRepository $demandeEmploiRepository,QuizRepository $quizRepository,OffreEmploiRepository $offreEmploiRepository, AdminRepository $adminRepository,AdminEmploiRepository $adminEmploiRepository,$id,Request $request): Response
    {
        ...
        $quiz=$quizRepository->findOneBy(['offre_emploi'=>$OffreEmploi]);
        $demandes=$demandeEmploiRepository->findBy(['OffreEmploi'=>$OffreEmploi]);
    ...
    public function showEmploi(Request $request,PaginatorInterface $paginator,OffreEmploiRepository $offreEmploiRepository,OffreStageRepository $offreStageRepository,AdminEmploiRepository $adminEmploiRepository,AdminRepository $adminRepository){
        ...
        $countStage=$em2->countNbOffre();
        $count=$em->countNbOffre();
        return $this->render('Admin_emploi/ShowDoneEmploi.html.twig',['ListeOffreEmploi'=>$AllOffreEmploi,'ListeOffreStage'=>$AllOffreStage,'admin'=>$admin,'c'=>$count,'cs'=>$countStage]);
    }
